...
def user_post():
    dropdownTo = request.form.get("dropdown1")
    dropdownFrom = request.form.get("dropdown2")
    messageContent = request.form.get("messageTextArea")
...
def userinfo_get():
    users = list(db.userinfo.find({}, {"_id": True}))
    all_info = list(
        db.userinfo.find(
            {},
            {"_id": True, "To": True, "From": True, "Message Content": True},
        )
    )
